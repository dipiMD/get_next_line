...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/25 13:19:45 by drakan            #+#    #+#             */
/*   Updated: 2021/10/25 13:49:14 by drakan           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoin_bonus(char const *s1, char const *s2)
{
	...
	if (!s1 || !s2)
		return (NULL);
	str = (char *)malloc(sizeof(*s1) * ft_strlen_bonus((char *)s1)
			+ ft_strlen_bonus((char *)s2) + 1);
	if (!str)
		return (NULL);
	while (s1[i])
	{
